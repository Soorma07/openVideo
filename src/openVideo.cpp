...
int main( int argc, char** argv )
{
    ...
    while( 1 ) //Infinite loop, mind termination conditions.
    {
        ...
        switch( optchr )
        {
            case 'l':
            case 'L':
                if( std::strcmp( long_options[ option_index ].name, "leftCamera" ) == 0 )
                {
    ...
    std::cout << bob::bobit << std::endl;
        if( !captureLeft.isOpened() )
        {
        	bob::cameraException camExcept1( "Stuff", 0);
            throw camExcept1;
        }
        ...
        {
            //bob::cameraExcpetion camExcept2;
            //throw camExcept2;
        }
